...
const UpdateControls: React.FunctionComponent<{ [label: string]: any }> = (
  ...
  const [filesMenu, setFilesMenu] = useState<any>([]);
  const [filesUrlMenu,setFilesUrlMenu]= useState<any>([])
  const stopPlay =
  ...
  const [loadingAfterModifyingQuiz, setLoadingAfterModifyingQuiz] =
    useState(false);
  ...
  const [selectedChannelOwner, setSelectedChannelOwner] =
        ...
        .then((res: any) => {
          if (res.data && res.data.channel.findBySchoolId) {
            ...
            const c = res.data.channel.findBySchoolId.filter((item: any) => {
              return (
                item.createdBy.toString().trim() !== userId.toString().trim()
              );
            });
            setOtherChannels(c);
            ...
            c.map((channel: any) => {
              if (!otherChannelOwnersMap[channel.createdBy]) {
                otherChannelOwnersMap[channel.createdBy] =
  ...
  const onSubmit = useCallback((ann: any) => {}, [annotations]);
  ...
  const onClickFile = (item: any) => {
    let tempObj = [];
    for (var i = 0; i < filesMenu.length; i++) {
      if (filesMenu[i].url === item.url) {
        tempObj.push({
          name: filesMenu[i].name,
          status: "active",
          type: filesMenu[i].type,
          url: filesMenu[i].url,
        });
      } else {
        tempObj.push({
    ...
    setFilesMenu(tempObj);
  };


 const onClickFileUrl=(item:any)=>{
    let tempObj = [];
    for (var i = 0; i < filesUrlMenu.length; i++) {
      if (filesUrlMenu[i].url === item.url) {
        tempObj.push({
          name: filesUrlMenu[i].name,
          status: "active",
          type: filesUrlMenu[i].type,
          url: filesUrlMenu[i].url,
        });
      } else {
        tempObj.push({
          name: filesUrlMenu[i].name,
    ...
    if (props.cue.channelId && props.cue.channelId !== "") {
      ...
      if (
        ...
        if (obj.quizId) {
          ...
          server
            ...
            .then((res) => {
              if (res.data && res.data.quiz.getQuiz) {
                ...
                setShuffleQuiz(
                  res.data.quiz.getQuiz.shuffleQuiz ? true : false
                );
                setTitle(obj.title);
                setIsQuiz(true);
                setInstructions(
         ...
         // setUrl(obj.url);
         console.log('again setting url')
          setFilesUrlMenu(obj.url)
          setFilesMenu(obj.url)
          setKey(Math.random());
        }
      } else {
        setImported(false);
  ...
  const handleUpdate = useCallback(async () => {
    ...
    subCues[props.cueKey][props.cueIndex] = {
      ...
      releaseSubmission: props.cue.releaseSubmission
  ...
  const handleSubmit = useCallback(async () => {
    ...
    Alert(
      ...
      now >= deadline
        ? "The deadline for this submission has already passed"
        : "",
      [
        {
          text: "Cancel",
    ...
    server
      .mutate({
        ...
        variables: {
          ...
          customCategory,
          shuffle,
          createdBy: selectedChannelOwner
            ? selectedChannelOwner.id
            : props.cue.createdBy,
  ...
  const renderRichToolbar = () => {
    return (props.cue.channelId &&
      ...
      !isOwner &&
      props.showOriginal) ||
      (props.showOriginal && showImportOptions) ||
      isQuiz ? (
      <View style={{ height: 0, backgroundColor: "#fff" }} />
    ) : (props.cue.graded && submission && !isOwner && !props.showOriginal) ||
      (!props.showOriginal && showImportOptions) ? (
      <View style={{ height: 0, backgroundColor: "#fff" }} />
    ) : (
      <RichToolbar
        ...
        iconMap={{
          ["insertCamera"]: ({ tintColor }) => (
            <Ionicons name="camera-outline" size={15} color={tintColor} />
          ),
          ["clear"]: ({ tintColor }) => (
            <Ionicons
              name="trash-outline"
      ...
      >
        ...
        >
          <Text style={{ flex: 1, fontSize: 11, color: "#a2a2ac" }}>
            ...
            prod → Product, sqrt → Square root, bar → Bar over letter, alpha,
            beta, ... omega → Small Greek letter, Alpha, Beta, ... Omega →
    ...
    return (
      <View style={{ flexDirection: "row" }}>
        ...
        >
          <Text
            style={
              !props.showOptions && props.showOriginal && !props.showComments
                ? styles.allGrayFill
                : styles.all
            }
          >
        ...
        {(isOwner && submission) || isQuiz ? null : (
          ...
          >
            <Text
              style={
                !props.showOriginal && !props.viewStatus
                  ? styles.allGrayFill
                  : styles.all
              }
            >
      ...
      <View style={{ flexDirection: "row", marginRight: 0, marginLeft: 0 }}>
        ...
        {isQuiz && !props.cue.graded ? (
          ...
          >
              ...
              {filesUrlMenu && filesUrlMenu.length > 0
                ? filesUrlMenu.map((item: any, key: number) => (
                    <>
                      <TouchableOpacity
                        style={{
                          maxWidth: 400,
        ...
        filesUrlMenu && filesUrlMenu.length > 0 ? (
            filesUrlMenu
              ...
              .map((u: any) =>
                ...
                u.type === "mp2" ||
                u.type === "wav" ? (
                  <ReactPlayer url={u.url} controls={true} />
                ) : (
                  <View style={{ width: 800, height: 50000 }}>
                    <Webview key={u.url} url={u.url} fullScreen={true} />
                  </View>
                )
              )
          ) :''
          ...
          <View style={{ width: "42.5%", backgroundColor: "white" }}>
            ...
            >
              <MenuTrigger>
                ...
                >
                  {selectedChannelOwner === undefined
                    ? "All"
                    : selectedChannelOwner !== null
                    ? selectedChannelOwner.name
                    : "Your Channels"}
                  <Ionicons name="caret-down" size={14} />
                </Text>
              </MenuTrigger>
              ...
              >
                {role === "instructor" ? (
                  <MenuOption value={undefined}>
                    <Text>All channels</Text>
                  </MenuOption>
                ) : null}
                <MenuOption value={null}>
                  <Text>Your channels</Text>
                </MenuOption>
                {channelOwners.map((own: any) => {
                  return (
                    <MenuOption value={own}>
                      <Text>{own.name}</Text>
                    </MenuOption>
                  );
                })}
          ...
          <View style={{ width: "42.5%", backgroundColor: "white" }}>
            ...
            >
              <MenuTrigger>
                ...
                >
                  {shareWithChannelName === "" ? "None" : shareWithChannelName}
                  <Ionicons name="caret-down" size={14} />
                </Text>
              </MenuTrigger>
              <MenuOptions
              ...
              >
                ...
                {selectedChannelOwner !== null
                  ? otherChannels.map((channel: any) => {
                      if (
                        selectedChannelOwner === undefined ||
                        channel.createdBy === selectedChannelOwner.id
                      ) {
                        return (
      ...
      >
        ...
        {props.cue.channelId && props.cue.channelId !== "" ? (
          ...
          <View style={{ flexDirection: "row" }}>
            ...
            >
              <Text
                style={!props.showOptions ? styles.allGrayFill : styles.all}
              >
                {PreferredLanguageText("viewShared")}
              </Text>
            </TouchableOpacity>
            ...
            <View style={{ backgroundColor: "#fff", flexDirection: "row" }}>
              ...
              !isQuiz ? (
                ...
                (!props.showOriginal &&
                  isOwner &&
                  props.cue.channelId &&
                  props.cue.channelId !== "") || // no submission as owner
                (!props.showOriginal && submissionImported && !isOwner) || // submitted as non owner
                (!props.showOriginal &&
                  !submission &&
                  props.cue.channelId &&
                  props.cue.channelId !== "") || // my notes
                isQuiz ? null : (
                <Text
                  style={{
                    color: "#2f2f3c",
          ...
          {props.showOptions || props.showComments ? null : (
            <View>
                  ...
                  {props.cue.submittedAt && props.cue.submittedAt !== "" ? (
                    ...
                    >
                      {/* <View style={{ marginRight: 25 }}>
                                    ...
                                    {filesUrlMenu.filter((k:any)=>k.status==='active').map((u:any)=>(
                                        <a
                              download={true}
                              href={
                                 u.url
                    ...
                    <View>
                    {filesMenu && filesMenu.length > 0
                      ? filesMenu.map((item: any, key: number) => (
                          <>
                            <TouchableOpacity
                              style={{
                                maxWidth: 400,
              ...
              {submissionImported || imported ? (
                ...
                <RichEditor
                  key={
                    props.showOriginal.toString() + reloadEditorKey.toString()
                  }
                  disabled={true}
                  containerStyle={{
                    display: "none",
